...
from interference_game.utils.metrics import build_candidate_actions, random_feasible_actions, spearman_rank_correlation
...
def run_from_config(config_path: str | Path) -> Path:
    ...
    for case in cases:
        ...
        for sample_idx, joint_action in enumerate(samples):
            ...
            for agent_idx in range(case.model_config.num_agents):
                ...
                exact_index = int(torch.tensor(exact_scores).argmax().item())
                    ...
                    raw_rows.append(
                        {
                            **case.metadata(),
                            ...
                            "accuracy": float(model_index == exact_index),
                            "rank_correlation": spearman_rank_correlation(exact_scores, scores),
                            "exact_best_index": exact_index,
    ...
    summary = (
        raw.groupby(["scenario", "model_name"], dropna=False)["accuracy"]
        .mean()
        .reset_index()
